    ...
    fn update(&mut self, tick: u64) {
        ...
        if tick % 5 == 0 && self.ships.len() < MAX_SHIPS {
            ...
            if frien_count < self.ships.len() / 2 {
                self.ships
                    .push(Ship::new(rand::thread_rng().gen_range(0..20), 73, true));
            } else {
                // too many friens, need enemies >:C
                self.ships
                    .push(Ship::new(rand::thread_rng().gen_range(0..20), 0, false));
            }
        }
        ...
        for (i, ship) in ships.iter_mut().enumerate() {
            if ship_remove_indices.contains(&i) {
                continue;
            }
            ...
            for (oi, os) in ships_cloned.iter().enumerate() {
                ...
                if os.going_up == ship.going_up {
                    continue;
                }
                if ship_remove_indices.contains(&oi) {
                    continue;
                }
        ...
        for (i, bullet) in bullets.iter_mut().enumerate() {
            if bullet_remove_indices.contains(&i) {
                continue;
            }
            ...
            for (oi, ob) in bullets_cloned.iter().enumerate() {
                ...
                if ob.going_up == bullet.going_up {
                    continue;
                }
                if bullet_remove_indices.contains(&oi) {
                    continue;
                }
            ...
            for (si, ship) in self.ships.iter().enumerate() {
                ...
                if ship_remove_indices.contains(&si) {
                    continue;
                }
                if ship.going_up == bullet.going_up {
                    continue;
                }
        ...
        for ind in bullet_remove_indices.iter().rev() {
            ...
            if bullets.len() == 0 || *ind >= bullets.len() {
                continue;
            }
...
impl Ship {
    ...
    fn update(&mut self, _tick: u64, game: &mut SpaceFight) {
        ...
        if self.shoot_timer == SHIP_SHOOT_TIMER {
            self.shoot_timer = 0;
            game.bullets
                .push(Bullet::new(self.x, self.y, self.going_up));
        }
